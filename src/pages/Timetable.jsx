  ...
  const fetchMovieTitles = async (showtimes) => {
    ...
    for (const showtime of showtimes) {
      if (!titles[showtime.movie_id]) {
        const response = await fetch(`${url}${showtime.movie_id}`);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  const fetchTheaterNames = async (showtimes) => {
    ...
    for (const showtime of showtimes) {
      if (!names[showtime.theater_id]) {
        const response = await fetch(`${url}${showtime.theater_id}`);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  const handleDayChange = (e) => {
    ...
    if (day === "") {
      ...
      const filtered = showtime.filter((showing) => {
        ...
        const showtimeDay = showtimeDate.toLocaleDateString("en-GB", {
          weekday: "long",
        });
  ...
  return (
    <>
      <div className="timetableCont">
        ...
        {error && (
          <p style={{ color: "red" }}>Failed to fetch showtimes: {error}</p>
        )}
            ...
            <tbody>
              {filteredShowtimes.map((showtime) => (
                <tr key={showtime.id}>
                  ...
                  <td>
                    {new Date(showtime.start_time).toLocaleDateString("bs-BA", {
                      weekday: "long",
                    })}
                  </td>
                  <td>
                    {new Date(showtime.end_time).toLocaleTimeString("bs-BA")}
                  </td>
                  <td>
